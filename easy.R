...
# Sample points uniformly from the cube using different random walks
step = 250
...
walks <- data.frame(transform(walks, X3 = as.factor(X3)))

# Plot the projections
p <- ggplot(walks, aes(X1, X2, colour=X3)) + geom_point() + scale_colour_manual("Dataset", values = c("blue", "red", "green"))
print(p)
